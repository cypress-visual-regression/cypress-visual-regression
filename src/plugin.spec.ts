...
const baseUpdateOptions: UpdateSnapshotOptions = {
  ...
  specName: 'sub-folder',
  screenshotAbsolutePath: path.join('tests/integration/cypress/fixtures/assets/base/enjuto.png'),
  baseDirectory: path.join('tests/integration/cypress/fixtures/assets/base'),
  spec: {
    name: 'enjuto.png',
    absolute: '',
    relative: ''
...
const baseCompareOptions: CompareSnapshotOptions = {
  ...baseUpdateOptions,
  errorThreshold: 0.1,
  diffDirectory: path.join('tests/integration/cypress/snapshots/test/diff'),
  generateDiff: 'fail'
}
  ...
  describe('generateImage', () => {
    ...
    describe('when the image is not generated', () => {
      it('should not generate an image and throw an error on directory creation', async () => {
        const invalidDir = path.join('tests/integration/ass\0ets*/invalidDir.png')
  ...
  describe('updateSnapshot', () => {
    ...
    describe('when there is an error on updating the snapshot', () => {
      it('should throw an error if cannot copy file', async () => {
        const options: UpdateSnapshotOptions = {
          ...baseUpdateOptions,
          screenshotAbsolutePath: path.join('fixtures/assets/wadus.png')
        }
        const result = updateSnapshot(options)
        const fullPath = path.join(options.baseDirectory ?? '', `${options.screenshotName}.png`)
        await expect(result).rejects.toThrow(
          `Failed to copy file from '${path.join('fixtures/assets/wadus.png')}' to '${fullPath}'.`
        )
      })
    })
  })
      ...
      describe('when image differs', () => {
        it('should not generate a diff image if generateDiff is set to never', async () => {
          const options: CompareSnapshotOptions = {
            ...baseCompareOptions,
            screenshotAbsolutePath: path.join('tests/integration/cypress/fixtures/assets/mod/never.png'),
            errorThreshold: 0,
            specName: '',
            screenshotName: 'never',
            generateDiff: 'never'
          }
          ...
          const isDiffGenerated = existsSync(path.join(options.diffDirectory as string, 'never.png'))
        ...
        it('should generate a diff image if generateDiff is set to always', async () => {
          const options: CompareSnapshotOptions = {
            ...baseCompareOptions,
            specName: '',
            screenshotAbsolutePath: path.join('tests/integration/cypress/fixtures/assets/mod/enjuto.png'),
            errorThreshold: 0,
            generateDiff: 'always'
          }
          const result = await compareSnapshots(options)
