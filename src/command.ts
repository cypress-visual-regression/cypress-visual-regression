...
function addCompareSnapshotCommand(screenshotOptions?: ScreenshotOptions): void {
  Cypress.Commands.add(
    ...
    function (subject: keyof HTMLElementTagNameMap, name: string, commandOptions?: PluginCommandOptions) {
      addCommand(Cypress, cy, subject, name, commandOptions, screenshotOptions)
    }
  )
}
...
export function addCommand(
  ...
  return takeScreenshot(cy, subject, folderAndName, regressionOptions).then(function (screenShotProps) {
    console.log(this)
    console.log(screenShotProps)
...
function compareScreenshots(options: VisualRegressionOptions): Cypress.Chainable {
  return cy.task<VisualRegressionResult>('compareSnapshots', options).then((results: VisualRegressionResult) => {
